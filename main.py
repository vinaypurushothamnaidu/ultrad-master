...
loaders_ = {
	'master':master_train_loader,
	'master-val':master_test_loader
}
...
def train(net, loader_train, loader_test, optim):
	...
	for step in range(start_step+1, opt.steps+1):
		...
		print(f'\rtrain loss : {loss.item():.5f}| step :{step}/{opt.steps}|lr :{lr :.4f} |time_used :{(time.time()-start_time)/60 :.1f}',end='',flush=True)
